...
\item \textbf{ShopManager :} Tout comme l'\textbf{UserManager}, ce Composant contient des Services qui seront - à priori - destinés à une utilisation interne à l'application. Il sera utilisé lors de l'ajout des produits au \guim{Panier} d'un client (appelé \guim{Cart}, en Anglais), et pour valider les achats des clients. C'est également ce Composant qui pourra faire le lien avec les Services de paiements externes (non-implémentés dans notre prototype).
\end{itemize}
